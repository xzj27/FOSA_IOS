...
@interface SealerTable ()<UITableViewDataSource, UITableViewDelegate, SealerSectionViewDelegate>{
}
@property(nonatomic, strong) NSMutableArray *classArr;
@property(nonatomic, strong) NSMutableDictionary *switchDict;
@end

@implementation SealerTable
...
- (void)processData{
    ...
    for (int i = 0; i < [[self dataArr] count]; i ++) {
        SealerModel *model = [[self dataArr] objectAtIndex:i];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([[self classArr] count] >= indexPath.section) {
        ...
        SealerModel *model = [tmpArr objectAtIndex:indexPath.row];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    SealerHeader *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:headerID];
    if (!headerView) {
        headerView = [[SealerHeader alloc] initWithReuseIdentifier:headerID];
    }
