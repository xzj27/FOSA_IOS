...
#pragma mark - 更新数据
-(void) UpdateInfo{
    NSString *sql = [NSString stringWithFormat:@"UPDATE Fosa2 SET foodName = '%@',aboutFood = '%@',expireDate = '%@',remindDate = '%@',photoPath = '%@'  WHERE deviceName = '%@'",self.foodName.text,self.aboutFood.text,expire_Date,remind_Date,self.foodName.text,self.deviceID];
